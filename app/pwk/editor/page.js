...
pwk.Page.prototype.checkPageOverflow = function() {
  ...
  let contentEl = this.content_.getElement();
  let contentSize = googStyle.getSize(contentEl);
  let maxContentHeight = this.getMaxContentSize();

  // If page content higher than allowed value
  if (contentSize.height > maxContentHeight) {
    let nodes = []; // Nodes which will be moved to the next page
    let pageIndex = doc.indexOfPage(this);
    let pageNodes = doc.getPaginationIndex()[pageIndex];
    let pageNodesLength = pageNodes.length;
    let loopNode;
    let newLinkedNode;
    ...
    while (pageNodesLength--) {
      ...
      switch (loopNode.getType()) {
        case pwk.NodeTypes.PARAGRAPH:
          let leafNode = /** @type {pwk.LeafNode} */(loopNode);
          let nodeHeight = googStyle.getSize(leafNode.getElement()).height;
          let linkedNode = leafNode.getNextLinkedNode();

          // Is required to move the whole node?
          if ((contentEl.offsetHeight - nodeHeight) < maxContentHeight) {
            let lineLength = leafNode.getLinesCount();
            let loopLine;
            let heightToCut = 0;
            ...
            if (lineLength == 0) {
              ...
              if (!goog.isDefAndNotNull(linkedNode)) {
                ...
                for (let i = leafNode.getLinesCount() - 1;
                     i >= lineLength; i--) {
